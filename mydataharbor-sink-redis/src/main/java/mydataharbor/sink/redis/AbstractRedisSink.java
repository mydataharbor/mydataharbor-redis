...
public abstract class AbstractRedisSink<R, S extends BaseSettingContext> implements IDataSink<R, S> {
  ...
  public AbstractRedisSink(RedisConfig redisConfig) {
    this.redisConfig = redisConfig;
    Set<RedisURI> uris = new HashSet<>();
    for (int i = 0; i < redisConfig.getHost().size(); i++) {
      RedisURI.Builder builder = RedisURI.builder()
        .withTimeout(Duration.ofMillis(redisConfig.getTimeout()));
      if (redisConfig.isEnableAuth()) {
        builder.withPassword((CharSequence) redisConfig.getAuth());
      }
      if (redisConfig.getRedisClusterType().equals(RedisConfig.RedisClusterType.SENTINEL)) {
        builder.withSentinel(redisConfig.getHost().get(i), redisConfig.getPort().get(i));
        builder.withSentinelMasterId(redisConfig.getMasterId());
      } else {
        builder.withHost(redisConfig.getHost().get(i)).withPort(redisConfig.getPort().get(i));
      }
      RedisURI redisURI = builder.build();
      uris.add(redisURI);
    }
    switch (redisConfig.getRedisClusterType()) {
      case SINGLE:

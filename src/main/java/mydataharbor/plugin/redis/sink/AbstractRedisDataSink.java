  ...
  public AbstractRedisDataSink(RedisDataSinkConfig redisDataSinkConfig) {
    ...
    for (int i = 0; i < redisDataSinkConfig.getHost().size(); i++) {
      RedisURI.Builder builder = RedisURI.builder()
        .withTimeout(Duration.ofMillis(redisDataSinkConfig.getTimeout()));
      if (redisDataSinkConfig.isEnableAuth()) {
        builder.withPassword((CharSequence) redisDataSinkConfig.getAuth());
      }
      if (redisDataSinkConfig.getRedisClusterType().equals(RedisDataSinkConfig.RedisClusterType.SENTINEL)) {
        builder.withSentinel(redisDataSinkConfig.getHost().get(i), redisDataSinkConfig.getPort().get(i));
        builder.withSentinelMasterId(redisDataSinkConfig.getMasterId());
      } else {
        builder.withHost(redisDataSinkConfig.getHost().get(i)).withPort(redisDataSinkConfig.getPort().get(i));
      }
